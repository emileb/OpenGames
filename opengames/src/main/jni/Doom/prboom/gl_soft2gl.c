...
void gld_Init8InGLMode(void)
{
  ...
  if (vid_8ingl.texid)
  {
    qglDeleteTextures(1, &vid_8ingl.texid);
    vid_8ingl.texid = 0;
  }
  ...
  qglViewport(0, 0, REAL_SCREENWIDTH, REAL_SCREENHEIGHT);
  gld_Set2DMode();
  ...
  qglGenTextures(1, &vid_8ingl.texid);

  qglBindTexture(GL_TEXTURE_2D, vid_8ingl.texid);
  ...
  // qglBufferDataARB with NULL pointer reserves only memory space.
...
void gld_Draw8InGL(void)
{
  ...
  if (gl_arb_pixel_buffer_object)
  {
    ...
    // Note that qglMapBufferARB() causes sync issue.
    // If GPU is working with this buffer, qglMapBufferARB() will wait(stall)
    // for GPU to finish its job. To avoid waiting (stall), you can call
    // first qglBufferDataARB() with NULL pointer before qglMapBufferARB().
    // If you do that, the previous data in PBO will be discarded and
    // qglMapBufferARB() returns a new allocated pointer immediately
  ...
  else
  {
    ...
    GL_RGBA, GL_UNSIGNED_BYTE, vid_8ingl.buf);
  }

  qglBegin(GL_TRIANGLE_STRIP);
  {

    qglTexCoord2f(vid_8ingl.fU1, vid_8ingl.fV1);
    qglVertex2f(0.0f, 0.0f);
    
    qglTexCoord2f(vid_8ingl.fU1, vid_8ingl.fV2);
    qglVertex2f(0.0f, (float)REAL_SCREENHEIGHT);


    qglTexCoord2f(vid_8ingl.fU2, vid_8ingl.fV1);
    qglVertex2f((float)REAL_SCREENWIDTH, 0.0f);

    qglTexCoord2f(vid_8ingl.fU2, vid_8ingl.fV2);
    qglVertex2f((float)REAL_SCREENWIDTH, (float)REAL_SCREENHEIGHT);
