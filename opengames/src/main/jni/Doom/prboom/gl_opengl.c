...
void gld_InitOpenGLVersion(void)
{
  ...
  if (sscanf((const char*)qglGetString(GL_VERSION), "%d.%d", &MajorVersion, &MinorVersion) == 2)
  {
    if (MajorVersion > 1)
    {
...
{
#ifdef ANDROID
	//Also do here for software mode
  QGL_Init(0);
#endif
...
void SetTextureMode(tex_mode_e type)
{
  ...
  if (type == TM_MASK)
  {
    ...
    qglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA, GL_TEXTURE); //ANDROID
  ...
  else if (type == TM_OPAQUE)
  {
    ...
    qglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB, GL_TEXTURE); //ANDROID
  ...
  else if (type == TM_INVERT)
  {
    ...
    qglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB, GL_TEXTURE);//ANDROID
    qglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_RGB, GL_PRIMARY_COLOR);
    qglTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_RGB, GL_ONE_MINUS_SRC_COLOR);
    qglTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_RGB, GL_SRC_COLOR);
