...
void gld_SetupFloodStencil(GLWall *wall)
{
  ...
  // Create stencil 
  qglStencilFunc(GL_EQUAL, recursion, ~0); // create stencil
  qglStencilOp(GL_KEEP, GL_KEEP, GL_INCR); // increment stencil of valid pixels
  qglColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE); // don't write to the graphics buffer
  ...
  qglStencilFunc(GL_EQUAL, recursion+1, ~0); // draw sky into stencil
  qglStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);   // this stage doesn't modify the stencil

  qglColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE); // don't write to the graphics buffer
...
void gld_ClearFloodStencil(GLWall *wall)
{
  ...
  qglStencilFunc(GL_EQUAL, recursion, ~0);
  gld_EnableTexture2D(GL_TEXTURE0_ARB, true);
