...
void gld_InitMapPics(void)
{
  ...
  while (am_icons[i].name)
  {
    ...
    if (lump != -1)
    {
      ...
      if (surf)
      {
        qglGenTextures(1, &am_icons[i].tex_id);
        qglBindTexture(GL_TEXTURE_2D, am_icons[i].tex_id);

        //if (gl_arb_texture_non_power_of_two)
        if (1)
        {
          qglTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);
          //qglTexImage2D(GL_TEXTURE_2D, 0, gl_tex_format, surf->w, surf->h, 0, GL_RGBA, GL_UNSIGNED_BYTE, surf->pixels);
          qglTexImage2D(GL_TEXTURE_2D, 0, gl_tex_format, 8,8, 0, GL_RGBA, GL_UNSIGNED_BYTE, surf->pixels);
        }
        else
        {
        	//EMILE TODO FIX!
        ...
        qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);//tex_filter[MIP_PATCH].min_filter);
        qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);//tex_filter[MIP_PATCH].mag_filter);
 ...
 // LOGI("gld_DrawNiceThings");
  ...
  for (i = 0; i < am_icon_count; i++)
  {
...
#if defined(USE_VERTEX_ARRAYS) || defined(USE_VBO)
    {
      ...
      qglVertexPointer(2, GL_FLOAT, sizeof(thing->v[0]), &thing->v[0].x);
      qglTexCoordPointer(2, GL_FLOAT, sizeof(thing->v[0]), &thing->v[0].u);
      qglColorPointer(4, GL_UNSIGNED_BYTE, sizeof(thing->v[0]), &thing->v[0].r);

      qglDrawArrays(GL_QUADS, 0, things->count * 4);
    }
#else
    for (i = 0; i < things->count; i++)
    {
      ...
      qglColor4ubv(&thing->v[0].r);

      qglBegin(GL_TRIANGLE_FAN);
      {
        qglTexCoord2f(thing->v[0].u, thing->v[0].v);
        qglVertex2f(thing->v[0].x, thing->v[0].y);
        qglTexCoord2f(thing->v[1].u, thing->v[1].v);
        qglVertex2f(thing->v[1].x, thing->v[1].y);
        qglTexCoord2f(thing->v[2].u, thing->v[2].v);
        qglVertex2f(thing->v[2].x, thing->v[2].y);
        qglTexCoord2f(thing->v[3].u, thing->v[3].v);
        qglVertex2f(thing->v[3].x, thing->v[3].y);
      }
      qglEnd();
    }
#endif
...
void gld_DrawMapLines(void)
{
  ...
  if (map_lines.count > 0)
  {
    ...
    qglVertexPointer(2, GL_FLOAT, sizeof(point[0]), &point->x);
    qglColorPointer(4, GL_UNSIGNED_BYTE, sizeof(point[0]), &point->r);

    qglDrawArrays(GL_LINES, 0, map_lines.count * 2);
