...
void gld_InitShadows(void)
{
  ...
  if (lump != -1)
  {
    ...
    if (surf)
    {
      qglGenTextures(1, &simple_shadows.tex_id);
      qglBindTexture(GL_TEXTURE_2D, simple_shadows.tex_id);

      qglTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, surf->w, surf->h, 0, GL_RGB, GL_UNSIGNED_BYTE, surf->pixels);
...
{
  qglColor3f(shadow->light, shadow->light, shadow->light);

  qglBegin(GL_TRIANGLE_FAN);

  qglTexCoord2f(1.0f, 0.0f);
  qglVertex3f(shadow->x + shadow->radius, shadow->z, shadow->y - shadow->radius);
...
void gld_RenderShadows(void)
{
  ...
  if (!gl_ztrick)
  {
    qglDepthRange(simple_shadows.bias, 1);
  }
  ...
  qglTranslatef(xCamera, zCamera, yCamera);
  qglScalef(0.99f, 0.99f, 0.99f);
  qglTranslatef(-xCamera, -zCamera, -yCamera);
