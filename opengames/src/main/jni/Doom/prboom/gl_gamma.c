...
void gld_CheckHardwareGamma(void)
{
  ...
  //gl_DeviceSupportsGamma = 0;
  //LOGI("gl_DeviceSupportsGamma = %d", gl_DeviceSupportsGamma);
...
int gld_SetGammaRamp(int gamma)
{
	...
	lprintf(LO_WARN,"gld_SetGammaRamp %d",gamma);
  ...
  lprintf(LO_WARN,"gld_SetGammaRamp suceeeded = %d",succeeded);
...
{
	lprintf(LO_WARN,"gld_ApplyGammaRamp");
