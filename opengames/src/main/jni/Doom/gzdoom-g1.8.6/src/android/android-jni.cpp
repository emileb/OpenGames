...
#define JAVA_FUNC(x) Java_com_beloko_opengames_gzdoom_NativeLib_##x
...
void swapBuffers()
{
	if (NativeLibClass == 0)
	{
		NativeLibClass = env_->FindClass("com/beloko/opengames/gzdoom/NativeLib");
...
JAVA_FUNC(touchEvent) (JNIEnv *env, jobject obj,jint action, jint pid, jfloat x, jfloat y)
{
	...
	LOGI("touchEvent %s  %d  x=%f",a,pid,x);
	*/
