...
EXTERN_CVAR (Bool, intermission_carry_powerups_an)
//===========================================================================
//
...
bool ABasicArmorPickup::Use (bool pickup)
{
	...
	else
	{
		...
		// use it.
		if (!intermission_carry_powerups_an)
	...
	if (!intermission_carry_powerups_an)
		...
		armor->Amount = armor->Amount + SaveAmount + armor->BonusCount;
...
bool AHexenArmor::AddArmorToSlot (AActor *actor, int slot, int amount)
{
	...
	if (amount <= 0)
	{
		...
		{
			Slots[slot] += hits; //"+=" operand should be used, not just "=", also you may define the local variable max_hexen_armor = 500
		}
	}
	else
	{
		...
		if (intermission_carry_powerups_an)
			max = SlotsIncrement[0]+SlotsIncrement[1]+SlotsIncrement[2]+SlotsIncrement[3]+Slots[4]+400*5*FRACUNIT;
