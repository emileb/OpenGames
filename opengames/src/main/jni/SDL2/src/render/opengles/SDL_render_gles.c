...
GLES_CreateTexture(SDL_Renderer * renderer, SDL_Texture * texture)
{
   ...
   // LOGI("w %d, h %d,",texture_w,texture_h);
                   ...
                   const SDL_Rect * rect, const void *pixels, int pitch)
{
    ...
    //LOGI("tex = %d, %d  %d  %d  %d", data->texture,rect->x,rect->y,rect->w,rect->h);
...
extern int Android_ScreenWidth_Actual,Android_ScreenHeight_Actual;
static int
GLES_UpdateViewport(SDL_Renderer * renderer)
{
    ...
    data->glViewport(renderer->viewport.x, renderer->viewport.y,
    		Android_ScreenWidth_Actual, Android_ScreenHeight_Actual);

    if (renderer->viewport.w && renderer->viewport.h) {
        data->glMatrixMode(GL_PROJECTION);
        data->glLoadIdentity();
        data->glOrthof((GLfloat) 0,
                 (GLfloat)Android_ScreenWidth_Actual,
                 (GLfloat)Android_ScreenHeight_Actual,
                 (GLfloat) 0, 0.0, 1.0);
    }

    LOGI("renderer->viewport.w %d",renderer->viewport.w );
                ...
                const SDL_Rect * srcrect, const SDL_FRect * dstrect)
{
    ...
    if (data->GL_OES_draw_texture_supported && data->useDrawTexture) {
        ...
        minx = dstrect->x;
        miny = dstrect->y;
        maxx = dstrect->x + Android_ScreenWidth_Actual;
        maxy = dstrect->y + Android_ScreenHeight_Actual;
