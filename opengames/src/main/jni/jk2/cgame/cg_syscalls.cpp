...
int (*syscallBack)( int arg, ... ) = (int (*)( int, ...))-1;

#ifdef __linux__
extern "C" {
#endif
void  __attribute__((visibility("default"))) dllEntryJk2( int (*syscallptr)( int arg,... ) ) {
...
void	cgi_CM_LoadMap( const char *mapname ) {
	syscallBack( CG_CM_LOADMAP, mapname );
}
...
void	cgi_S_AddLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx ) {
	syscallBack( CG_S_ADDLOOPINGSOUND, entityNum, origin, velocity, sfx );
}
...
{
	return syscallBack( CG_ANYLANGUAGE_READFROMSTRING, ppText, pbIsTrailingPunctuation );
}
...
{
	return(int) syscallBack(CG_UI_GETMENUINFO,menuFile,x,y);
}
...
{
	return syscallBack( CG_SP_REGISTER, text, persist );
}
...
{
	return syscallBack( CG_SP_GETSTRINGTEXT, ID, buffer, bufferLength );
}
...
//extern void CMD_CGCam_Disable( void );
	...
	return syscallBack( CG_SENDCONSOLECOMMAND, "cam_disable; set nextmap disconnect; cinematic outcast\n" );
}
