...
//		assert(VMA(2));	// can now pass in NULL to just query the size
		...
		return strlen(text);
		//break;
	case CG_ANDROID_SET_LOOK_SCALE:
	{
		extern int android_look_scale;
		android_look_scale = args[1];
		//LOGI("CG_ANDROID_SET_LOOK_SCALE %d",android_look_scale);
	}
		break;
	default:
