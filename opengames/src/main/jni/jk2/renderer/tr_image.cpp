...
extern "C"
{
//ETC1 compression copied from Quake for Raspberry Pi
 //WTF does not work for some reason..
   ...
   const GLvoid *pixels)
{
	...
	etc1_encode_image(pixels, width, height,3, width*3, etc1data);
...
void myglTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels,bool opaque)
{
   //static int opaque = 0;
   //LOGI("myglTexImage2D level=%d",level);
   if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
      assert(level == 0);
     // opaque = 1;
      LOGI("etc1_compress_tex_image GL_RGB");
      ...
      if (opaque)
      {
    	 //LOGI("etc1_compress_tex_image GL_RGBA level=%d",level);
    	 byte *temp;
    	 temp = gles_convertRGB((byte*)pixels, width, height);
         etc1_compress_tex_image(target, level, GL_RGB, width, height, border, GL_RGB, type, temp);
        // qglTexImage2D (target, level, GL_RGB, width, height, border, GL_RGB, type, temp);
         ri.Free(temp);
						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	*pUploadHeight = height;
	...
	if (r_picmip->integer < 2) //Compress textures if picmip < 2
	{
		byte *temp;
		switch ( *pformat ) {
		/*
		...
		case 3:
		case GL_RGB:
			myglTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, temp);
			break;
			*/
		case 1:
		...
		default:
			*pformat = GL_RGBA;
			myglTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data, samples==3?true:false);
			...
			if (mipmap)
			{
				...
				while (width > 1 || height > 1)
				{
					...
					myglTexImage2D (GL_TEXTURE_2D, miplevel, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data, samples==3?true:false);
...
//ri.Printf(PRINT_ALL, "R_CreateImage(\"%s\", %p, %i, %i, %i, %i, %i, %i)\n", name, pic, width, height, mipmap, allowPicmip, allowTC, glWrapClampMode);
	...
	if (name[0] == '*')
	{
		char *psLightMapNameSearchPos = (char*) strrchr(name,'/');
