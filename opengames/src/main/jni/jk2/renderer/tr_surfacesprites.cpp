...
static void R_SurfaceSpriteFrameUpdate(void)
{
	...
	{	// This isn't initialized yet.
		if (backEnd.refdef.fov_x > 50 && backEnd.refdef.fov_x < 135)		// I don't consider anything below 50 or above 135 to be "normal".
		{
			...
			standardscalex = tanf(standardfovx * 0.5f * (M_PI/180.0f));
		...
		rangescalefactor = 1.0f;		// Don't multiply the shader range by anything.
	}
	...
	{	// We are using a non-standard FOV.  We need to multiply the range of the shader by a scale factor.
		...
		{
			rangescalefactor = standardscalex / tanf(135.0f * 0.5f * (M_PI/180.0f));
		}
		else
		{
			rangescalefactor = standardscalex / tanf(backEnd.refdef.fov_x * 0.5f * (M_PI/180.0f));
		}
	}
...
//		VectorScale(targetWindGrassDir, targetspeed, targetWindGrassDir);
	...
	dtime = (float)(backEnd.refdef.time - lastSSUpdateTime) * (1.0f/(float)WIND_DAMP_INTERVAL);	// Our dampfactor is geared towards a time interval equal to "1".
										...
										byte alpha, float wind, float windidle, vec2_t fog, int hangdown, vec2_t skew)
{
	...
	if (wind>0.0f && curWindSpeed > 0.001f)
	{
		...
		{
			windsway *= curWindSpeed*(1.0f/100.0f);
		}
		else
		{
			windsway *= 0.4f;
...
//						light *= alpha;
					...
					if (usewindpoint && windforce > 0 && stage->ss.wind > 0.0f)
					{
						if (SSUsingFog)
						{
...
void RB_DrawSurfaceSprites( shaderStage_t *stage, shaderCommands_t *input) 
{
	...
	{	
		...
		{	// Drawing the world, so our job is dead-easy, in the viewparms
			VectorCopy(backEnd.viewParms.orient.origin, ssViewOrigin);
			VectorCopy(backEnd.viewParms.orient.axis[1], ssViewRight);
			VectorCopy(backEnd.viewParms.orient.axis[2], ssViewUp);
		}
		else
		{	// Drawing an entity, so we need to transform the viewparms to the model's coordinate system
//			R_WorldPointToEntity (backEnd.viewParms.orient.origin, ssViewOrigin);
			R_WorldNormalToEntity (backEnd.viewParms.orient.axis[1], ssViewRight); 
			R_WorldNormalToEntity (backEnd.viewParms.orient.axis[2], ssViewUp);
			VectorCopy(backEnd.orient.viewOrigin, ssViewOrigin);
//			R_WorldToLocal(backEnd.viewParms.orient.axis[1], ssViewRight);
//			R_WorldToLocal(backEnd.viewParms.orient.axis[2], ssViewUp);
		}
