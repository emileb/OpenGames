...
static void AutospriteDeform( void ) {
	...
	if ( tess.numVertexes & 3 ) {
		ri.Error( ERR_DROP, "Autosprite shader %s had odd vertex count", tess.shader->name );
	}
	if ( tess.numIndexes != ( tess.numVertexes >> 2 ) * 6 ) {
		ri.Error( ERR_DROP, "Autosprite shader %s had odd index count", tess.shader->name );
	}
...
void RB_CalcFogTexCoords( float *st ) {
	...
	VectorSubtract( backEnd.orient.origin, backEnd.viewParms.orient.origin, localVec );
	...
	fogDistanceVector[3] = DotProduct( localVec, backEnd.viewParms.orient.axis[0] );
