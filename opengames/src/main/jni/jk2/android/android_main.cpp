...
//This is defined in android-jni.cpp
extern "C"
{
...
*/
	...
	sprintf(lib_path,"%s/libjk2game.so", getLibPath());
	...
	if (!game_library) //Some devices put an extra 1 on the end!!!
	{
		...
		game_library = dlopen ("/data/data/com.beloko.jk2/lib/libjk2game.so", RTLD_LAZY );
	}

	if (!game_library) //Some devices put an extra 1 on the end!!!
	{
		LOGI("TRYING -1 LIBRARY");
		game_library = dlopen ("/data/data/com.beloko.jk2-1/lib/libjk2game.so", RTLD_LAZY );
	}

	if (!game_library) //Some devices put an extra 1 on the end!!!
	{
		LOGI("TRYING -2 LIBRARY");
		game_library = dlopen ("/data/data/com.beloko.jk2-2/lib/libjk2game.so", RTLD_LAZY );
	}

	if (!game_library) //Some devices put an extra 1 on the end!!!
	{
		LOGI("TRYING -3 LIBRARY");
		game_library = dlopen ("/data/data/com.beloko.jk2-3/lib/libjk2game.so", RTLD_LAZY );
	}
...
void * Sys_LoadCgame( int (**entryPoint)(int, ...), int (*systemcalls)(int, ...) ) 
{
	...
	dllEntry = ( void (*)( int (*)( int, ... ) ) )dlsym( game_library, "dllEntryJk2" );
