...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
		...
		case SE_MOUSE:
			...
			break;
		case SE_MOUSE_ABS:
			CL_MouseEventAbs( ev.evValue, ev.evValue2);
			break;
...
/*
		//Check Beta data
		if (com_frameNumber == 9000) //2 minutes
		{
			...
			// LOGI("%d  %d  %d",now->tm_year + 1900,now->tm_mon + 1,now->tm_mday);
		}
*/
