...
#define JAVA_FUNC(x) Java_com_beloko_opengames_gish_NativeLib_##x
...
void swapBuffers()
{
	if (NativeLibClass == 0)
	{
		NativeLibClass = env_->FindClass("com/beloko/opengames/gish/NativeLib");
...
{
	//PortableMouse(dx,dy);
	PortableMouseAbs(x * android_screen_width,y * android_screen_height);
...
void initControls(int width, int height,const char * graphics_path)
{
	...
	if (!controlsCreated)
	{
		...
		tcGameMain->addControl(new touchcontrols::Button("jump",touchcontrols::RectF(21,5,24,8),"fire2",SCAN_SPACE));
		tcGameMain->addControl(new touchcontrols::Button("heavy",touchcontrols::RectF(21,8,24,10),"use",SCAN_D));
		tcGameMain->addControl(new touchcontrols::Button("stick",touchcontrols::RectF(19,7,21,9),"use",SCAN_A));
		tcGameMain->addControl(new touchcontrols::Button("slide",touchcontrols::RectF(24,7,26,9),"use",SCAN_S));


		tcGameMain->addControl(new touchcontrols::Button("keyboard",touchcontrols::RectF(8,0,10,2),"keyboard",KEY_SHOW_KBRD));


		tcGameMain->addControl(new touchcontrols::Button("down_arrow",touchcontrols::RectF(3,13,6,16),"arrow_down",SCAN_DOWN));
		tcGameMain->addControl(new touchcontrols::Button("up_arrow",touchcontrols::RectF(3,10,6,13),"arrow_up",SCAN_UP));
		tcGameMain->addControl(new touchcontrols::Button("left_arrow",touchcontrols::RectF(0,13,3,16),"arrow_left",SCAN_LEFT));
		tcGameMain->addControl(new touchcontrols::Button("right_arrow",touchcontrols::RectF(6,13,9,16),"arrow_right",SCAN_RIGHT));
