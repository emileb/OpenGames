...
#ifdef __ANDROID__
extern "C"  int Android_GetMenuControl();
#endif
...
#ifdef __ANDROID__
    //Consume any old menu actions
      Android_GetMenuControl();
#endif
...
#ifdef __ANDROID__
        int menuCtrl = Android_GetMenuControl();
        l_win->inm->handle_controller_event(menuCtrl,ev);
#endif
        if (ev.type==EV_MESSAGE && ev.message.id>=ID_LOAD_GAME_NUMBER && ev.message.id<ID_LOAD_GAME_PREVIEW)
            got_level=ev.message.id-ID_LOAD_GAME_NUMBER+1;
