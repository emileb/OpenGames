...
*/
	static int xoffLast=0;
	static int yoffLast=0;

	if (interpToggle)
	{
		xoff = xoffLast + (float)(v->xoff() - xoffLast) * interpAmmount;
		yoff = yoffLast + (float)(v->yoff() - yoffLast) * interpAmmount;
	}
	else{
		xoff = v->xoff();
		yoff = v->yoff();
	...
	//LOGI("%d   %d  %d",interpToggle,xoff,yoff);
...
/*
	struct timeval tv;
	static long long msLast=0;
	gettimeofday(&tv, NULL);

	long long  msTime = ((tv.tv_sec) * 1000 + tv.tv_usec/1000.0) + 0.5;

	int diff = msTime - msLast;
	LOGI("diff = %d",diff);
	if (diff > 0 && diff < 30)
	{
 		usleep((30 - diff) * 1000);
...
*/
	...
	if (avg_ms < 1000.0f / (ANDROID_FPS-1))
	...
	else if (avg_ms < 1000.0f / ANDROID_FPS && need_delay)
	{
		frame_panic = 0;
		if (!no_delay)
		{
			frame_timer.WaitMs(1000.0f / ANDROID_FPS);
			avg_ms -= 0.1f * deltams;
			avg_ms += 0.1f * 1000.0f / ANDROID_FPS;
		}
	}
	else if (avg_ms > 1000.0f / (ANDROID_FPS-1))
	{
...
void Game::step()
{
	...
	if(state == RUN_STATE)
	{
		...
		else if(!(dev & EDIT_MODE))               // if edit mode, then don't step anything
		{
			...
			static int playerOnly=0;
			// if (playerOnly)
			//    playerOnly = 0;
			// else
			//    playerOnly = 1;

			current_level->tick(playerOnly);
