...
void InputManager::handle_controller_event(int action,event &ev)
{
	...
	if (!active)//nothing selected yet..
	{
		active = first;
	...
	int max_distance = 0xFFFFFFF;
	ifield * closest = NULL;

	active->area(ax1,ay1,ax2,ay2);
	if (action == 1) //UP
	{
		//Up
		for (i=first; i; i=i->next)
		{
			...
			{
				i->area(x1,y1,x2,y2);
				if (y1 < ay1) //Check if this is ABOVE active item
				{
					int dist = abs(x1 - ax1) + abs(y1 - ay1);
					if (dist < max_distance) //Find closest
					{
						closest = i;
						max_distance = dist;
					}
	...
	if (action == 2) //DOWN
	{
		for (i=first; i; i=i->next)
		{
			...
			{
				i->area(x1,y1,x2,y2);
				if (y1 > ay1) //Check if this is UNDER active item
				{
					int dist = abs(x1 - ax1) + abs(y1 - ay1);
					if (dist < max_distance) //Find closest
					{
	...
	if (action == 3) //LEFT
	{
		for (i=first; i; i=i->next)
		{
			...
			{
				i->area(x1,y1,x2,y2);
				if (x1 < ax1) //Check if this is LEFT of active item
				{
					int dist = abs(x1 - ax1) + abs(y1 - ay1);
					if (dist < max_distance) //Find closest
					{
	...
	if (action == 5) //SELECT
	{
		if (active)
		{
			...
			ev.mouse_move.x = ax1 + 1;
			ev.mouse_move.y = ay1 + 1;
