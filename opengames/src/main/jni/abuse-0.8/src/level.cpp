...
extern int interpToggle;
extern float interpAmmount;
...
int level::tick(int playerOnly)
{
	...
	for (o=first_active; o; )
	{
		...
		if (!(dev&SUSPEND_MODE) || c)
		{
			...
			else if (!playerOnly && !o->decide())      // if object returns 0, delete it... I don't like 0's :)
			{
				game_object *p=o;
				o=o->next_active;
				delete_object(p);
	...
	} else
	{
		if (0)
		{
			...
			if (interpToggle)
			{
				for (o=first_active; o; o=o->next_active)
				{
				//Hack to stop it glitching when warping
					if (abs(o->interp_last_x - o->x)> 20)
					{
						o->interp_last_x = o->x;
						o->interp_last_y = o->y;
					}
					...
					o->x = o->interp_last_x + (float)(xreal - o->interp_last_x) * interpAmmount;
					o->y = o->interp_last_y + (float)(yreal - o->interp_last_y) * interpAmmount;
