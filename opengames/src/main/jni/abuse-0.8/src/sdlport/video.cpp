...
void set_mode(int mode, int argc, char **argv)
{
    ...
    if (flags.gl)
    {
       ...
       // glPushAttrib(GL_ENABLE_BIT);
        ...
        int width,height;
        width = android_screen_width;
        height = android_screen_height;
...
flags.antialias = GL_NEAREST;
    ...
    surface = SDL_CreateRGBSurface(SDL_SWSURFACE, window->w, window->h, 8, 0, 0, 0, 0);
    if(surface == NULL)
    {
...
void update_window_done()
{
    ...
    if(flags.gl)
    {
        ...
        GLfloat vertices[] = {
            android_screen_width, 0, 0,
            0,0, 0,
            0, android_screen_height, 0,
            android_screen_width, android_screen_height, 0,
          };

        GLfloat tex1[] = {
             texcoord[2], 0.0,		// bottom left	(V1)
              0.0,  0,		// top left		(V2)
              0,  texcoord[3],		// top right	(V4)
              texcoord[2],texcoord[3]		// bottom right	(V3)
            };
