...
void *top_ai()
{
  ...
  if (o->total_objects())            // make sure we are linked to the main character
  {
   ...
   // LOGI ("best angle = %d",best_angle);
    best_angle = Android_GetAngle();
...
*/
    ...
    o->lvars[point_angle]=best_angle;
...
void *player_draw(int just_fired_var, int num)
{
  ...
  LOGI("cop x = %d y = %d", o->x-current_vxadd, o->y-current_vyadd);
  if (num==0)
  {
