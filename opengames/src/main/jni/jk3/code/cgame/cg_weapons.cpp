...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_SABER:
		...
		//theFxScheduler.RegisterEffect( "saber/limb_bolton" ); //LAvaPort - file is missing in Steam files
		theFxScheduler.RegisterEffect( "saber/fizz" );
		theFxScheduler.RegisterEffect( "saber/boil" );
		//theFxScheduler.RegisterEffect( "saber/fire" );//was "sparks/spark" //LAvaPort - file is missing in Steam files
...
//			assert(0); // no idea what to do here
	...
	*/
			...
			|| ( ps->weapon == WP_DEMP2 && ps->weaponstate == WEAPON_CHARGING_ALT ))
	{
		...
		FX_AddSprite( flash.origin, NULL, NULL, 3.0f * val * scale, 0.0f, 0.7f, 0.7f, WHITE, WHITE, randomLava() * 360, 0.0f, 1.0f, shader, FX_USE_ALPHA | FX_DEPTH_HACK );
	}
