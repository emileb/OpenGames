...
//		cgi_S_StartLocalSound ( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	...
	if ( !strcmp( cmd, "ct" ) ) 
	{
#ifdef __ANDROID__ //WTF This is needed otherwise subtitles dont work!!
		char *str = strdup(CG_Argv(1));
		LOGI("Got ct %s",str);
#endif
		CG_CaptionText(str, (int)cgs.sound_precache[atoi(CG_Argv(2))], (int)(SCREEN_HEIGHT * 0.25) );
#ifdef __ANDROID__
		free(str);
#endif
		return;
