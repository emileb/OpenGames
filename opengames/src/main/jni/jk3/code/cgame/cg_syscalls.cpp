...
static int (*syscallBack)( int arg, ... ) = (int (*)( int, ...))-1;
...
extern "C" {
#endif
void __attribute__((visibility("default"))) dllEntry( int (*syscallptr)( int arg,... ) ) {
#endif
...
void	cgi_CM_TransformedBoxTrace( trace_t *results, const vec3_t start, const vec3_t end,
	...
	syscallBack( CG_CM_TRANSFORMEDBOXTRACE, results, start, end, mins, maxs, model, brushmask, origin, angles );
}
...
void	cgi_FF_Xbox_Shake( float intensity, int duration ){
	syscallBack( CG_FF_XBOX_SHAKE, PASSFLOAT(intensity), duration );
}

void	cgi_FF_Xbox_Damage( int damage, float xpos ) {
	syscallBack( CG_FF_XBOX_DAMAGE, damage, PASSFLOAT(xpos) );
}

#endif

void	cgi_R_LoadWorldMap( const char *mapname ) {
	syscallBack( CG_R_LOADWORLDMAP, mapname );
}

qhandle_t cgi_R_RegisterModel( const char *name ) {
	return syscallBack( CG_R_REGISTERMODEL, name );
}

qhandle_t cgi_R_RegisterSkin( const char *name ) {
	return syscallBack( CG_R_REGISTERSKIN, name );
}

qhandle_t cgi_R_RegisterShader( const char *name ) {
	qhandle_t hShader = syscallBack( CG_R_REGISTERSHADER, name );
	assert (hShader);
	return  hShader;
}

qhandle_t cgi_R_RegisterShaderNoMip( const char *name ) {
	return syscallBack( CG_R_REGISTERSHADERNOMIP, name );
}

qhandle_t cgi_R_RegisterFont( const char *name ) {
	return syscallBack( CG_R_REGISTERFONT, name );
}

int cgi_R_Font_StrLenPixels(const char *text, const int iFontIndex, const float scale /*= 1.0f*/) {
	return syscallBack( CG_R_FONTSTRLENPIXELS, text, iFontIndex, PASSFLOAT(scale) ) ;
}

int cgi_R_Font_StrLenChars(const char *text) {
	return syscallBack( CG_R_FONTSTRLENCHARS, text ) ;
}

int cgi_R_Font_HeightPixels(const int iFontIndex, const float scale /*= 1.0f*/) {
	return syscallBack( CG_R_FONTHEIGHTPIXELS, iFontIndex, PASSFLOAT(scale) );
}

qboolean cgi_Language_IsAsian( void )
{
	return syscallBack( CG_LANGUAGE_ISASIAN );
}

qboolean cgi_Language_UsesSpaces(void)
{
	return syscallBack( CG_LANGUAGE_USESSPACES );
}
...
{
	return syscallBack( CG_ANYLANGUAGE_READFROMSTRING, psText, piAdvanceCount, pbIsTrailingPunctuation );
}
...
void cgi_R_Font_DrawString(int ox, int oy, const char *text, const float *rgba, const int setIndex, int iMaxPixelWidth, const float scale /*= 1.0f*/) {
	syscallBack (CG_R_FONTDRAWSTRING, ox, oy, text, rgba, setIndex, iMaxPixelWidth, PASSFLOAT(scale) );
}
...
{
	syscallBack(CG_R_SETREFRACTIONPROP, PASSFLOAT(alpha), PASSFLOAT(stretch), prepost, negate);
}

void	cgi_R_ClearScene( void ) {
	syscallBack( CG_R_CLEARSCENE );
}

void	cgi_R_AddRefEntityToScene( const refEntity_t *re ) {
	syscallBack( CG_R_ADDREFENTITYTOSCENE, re );
}

qboolean	cgi_R_inPVS( vec3_t p1, vec3_t p2 )
{
	return syscallBack( CG_R_INPVS, p1, p2 );
}


void	cgi_R_GetLighting( const vec3_t origin, vec3_t ambientLight, vec3_t directedLight, vec3_t ligthDir ) {
	syscallBack( CG_R_GETLIGHTING, origin, ambientLight, directedLight, ligthDir );
}

void	cgi_R_AddPolyToScene( qhandle_t hShader , int numVerts, const polyVert_t *verts ) {
	syscallBack( CG_R_ADDPOLYTOSCENE, hShader, numVerts, verts );
}

void	cgi_R_AddLightToScene( const vec3_t org, float intensity, float r, float g, float b ) {
	syscallBack( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b) );
}

void	cgi_R_RenderScene( const refdef_t *fd ) {
	syscallBack( CG_R_RENDERSCENE, fd );
}

void	cgi_R_SetColor( const float *rgba ) {
	syscallBack( CG_R_SETCOLOR, rgba );
}

void	cgi_R_DrawStretchPic( float x, float y, float w, float h, 
							   float s1, float t1, float s2, float t2, qhandle_t hShader ) {
	syscallBack( CG_R_DRAWSTRETCHPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), hShader );
}

//void	cgi_R_DrawScreenShot( float x, float y, float w, float h){
//	syscallBack( CG_R_DRAWSCREENSHOT, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h) );
//}

void	cgi_R_ModelBounds( qhandle_t model, vec3_t mins, vec3_t maxs ) {
	syscallBack( CG_R_MODELBOUNDS, model, mins, maxs );
}

void	cgi_R_LerpTag( orientation_t *tag, qhandle_t mod, int startFrame, int endFrame, 
					   float frac, const char *tagName ) {
	syscallBack( CG_R_LERPTAG, tag, mod, startFrame, endFrame, PASSFLOAT(frac), tagName );
}

void	cgi_R_DrawRotatePic( float x, float y, float w, float h, 
				   float s1, float t1, float s2, float t2,float a, qhandle_t hShader ) 
{
	syscallBack( CG_R_DRAWROTATEPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), PASSFLOAT(a), hShader );
}

void	cgi_R_DrawRotatePic2( float x, float y, float w, float h, 
				   float s1, float t1, float s2, float t2,float a, qhandle_t hShader ) 
{
	syscallBack( CG_R_DRAWROTATEPIC2, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), PASSFLOAT(a), hShader );
}

//linear fogging, with settable range -rww
void cgi_R_SetRangeFog(float range)
{
	syscallBack(CG_R_SETRANGEFOG, PASSFLOAT(range));
}

void	cgi_R_LAGoggles( void )
{
	syscallBack( CG_R_LA_GOGGLES );
}

void	cgi_R_Scissor( float x, float y, float w, float h) 
{
	syscallBack( CG_R_SCISSOR, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h));
}

void		cgi_GetGlconfig( glconfig_t *glconfig ) {
	syscallBack( CG_GETGLCONFIG, glconfig );
}

void		cgi_GetGameState( gameState_t *gamestate ) {
	syscallBack( CG_GETGAMESTATE, gamestate );
}

void		cgi_GetCurrentSnapshotNumber( int *snapshotNumber, int *serverTime ) {
	syscallBack( CG_GETCURRENTSNAPSHOTNUMBER, snapshotNumber, serverTime );
}

qboolean	cgi_GetSnapshot( int snapshotNumber, snapshot_t *snapshot ) {
	return syscallBack( CG_GETSNAPSHOT, snapshotNumber, snapshot );
}

qboolean	cgi_GetDefaultState(int entityIndex, entityState_t *state )
{
	return syscallBack( CG_GETDEFAULTSTATE, entityIndex, state );
}

qboolean	cgi_GetServerCommand( int serverCommandNumber ) {
	return syscallBack( CG_GETSERVERCOMMAND, serverCommandNumber );
}

int			cgi_GetCurrentCmdNumber( void ) {
	return syscallBack( CG_GETCURRENTCMDNUMBER );
}

qboolean	cgi_GetUserCmd( int cmdNumber, usercmd_t *ucmd ) {
	return syscallBack( CG_GETUSERCMD, cmdNumber, ucmd );
}

void		cgi_SetUserCmdValue( int stateValue, float sensitivityScale, float mPitchOverride, float mYawOverride ) {
	syscallBack( CG_SETUSERCMDVALUE, stateValue, PASSFLOAT(sensitivityScale), PASSFLOAT(mPitchOverride), PASSFLOAT(mYawOverride) );
}

void		cgi_SetUserCmdAngles( float pitchOverride, float yawOverride, float rollOverride ) {
	syscallBack( CG_SETUSERCMDANGLES, PASSFLOAT(pitchOverride), PASSFLOAT(yawOverride), PASSFLOAT(rollOverride) );
}
/*
Ghoul2 Insert Start
*/
...
{
	syscallBack( CG_G2_SETMODELS, &ghoul2, modelList, skinList);
}
/*
Ghoul2 Insert End
*/
...
{
	syscallBack(COM_SETORGANGLES,org,angles);
}

void	trap_R_GetLightStyle(int style, color4ub_t color)
{
	syscallBack(CG_R_GET_LIGHT_STYLE, style, color);
}

void	trap_R_SetLightStyle(int style, int color)
{
	syscallBack(CG_R_SET_LIGHT_STYLE, style, color);
}

void	cgi_R_GetBModelVerts(int bmodelIndex, vec3_t *verts, vec3_t normal )
{
	syscallBack( CG_R_GET_BMODEL_VERTS, bmodelIndex, verts, normal );
}

void	cgi_R_WorldEffectCommand( const char *command )
{
	syscallBack( CG_R_WORLD_EFFECT_COMMAND, command );
}
...
int trap_CIN_PlayCinematic( const char *arg0, int xpos, int ypos, int width, int height, int bits, const char *psAudioFile /* = NULL */) {
  return syscallBack(CG_CIN_PLAYCINEMATIC, arg0, xpos, ypos, width, height, bits, psAudioFile);
}
 
// stops playing the cinematic and ends it.  should always return FMV_EOF
// cinematics must be stopped in reverse order of when they are started
e_status trap_CIN_StopCinematic(int handle) {
  return (e_status) syscallBack(CG_CIN_STOPCINEMATIC, handle);
}


// will run a frame of the cinematic but will not draw it.  Will return FMV_EOF if the end of the cinematic has been reached.
e_status trap_CIN_RunCinematic (int handle) {
  return (e_status) syscallBack(CG_CIN_RUNCINEMATIC, handle);
}
 

// draws the current frame
void trap_CIN_DrawCinematic (int handle) {
  syscallBack(CG_CIN_DRAWCINEMATIC, handle);
}
 

// allows you to resize the animation dynamically
void trap_CIN_SetExtents (int handle, int x, int y, int w, int h) {
  syscallBack(CG_CIN_SETEXTENTS, handle, x, y, w, h);
}

void *cgi_Z_Malloc( int size, int tag )
{
	return (void *)syscallBack(CG_Z_MALLOC,size,tag);
}

void cgi_Z_Free( void *ptr )
{
	syscallBack(CG_Z_FREE,ptr);
}

void cgi_UI_SetActive_Menu(char *name)
{
	syscallBack(CG_UI_SETACTIVE_MENU,name);
}

void cgi_UI_Menu_OpenByName(char *buf)
{
	syscallBack(CG_UI_MENU_OPENBYNAME,buf);
}

void cgi_UI_Menu_Reset(void)
{
	syscallBack(CG_UI_MENU_RESET);
}

void cgi_UI_Menu_New(char *buf)
{
	syscallBack(CG_UI_MENU_NEW,buf);
}

void cgi_UI_Parse_Int(int *value)
{
	syscallBack(CG_UI_PARSE_INT,value);
}

void cgi_UI_Parse_String(char *buf)
{
	syscallBack(CG_UI_PARSE_STRING,buf);
}

void cgi_UI_Parse_Float(float *value)
{
	syscallBack(CG_UI_PARSE_FLOAT,value);
}

int cgi_UI_StartParseSession(char *menuFile,char **buf)
{
	return(int) syscallBack(CG_UI_STARTPARSESESSION,menuFile,buf);
}

void cgi_UI_EndParseSession(char *buf)
{
	syscallBack(CG_UI_ENDPARSESESSION,buf);
}

void cgi_UI_ParseExt(char **token)
{
	syscallBack(CG_UI_PARSEEXT,token);
}

void cgi_UI_MenuCloseAll(void)
{
	syscallBack(CG_UI_MENUCLOSE_ALL);
}

void cgi_UI_MenuPaintAll(void)
{
	syscallBack(CG_UI_MENUPAINT_ALL);
}

void cgi_UI_String_Init(void)
{
	syscallBack(CG_UI_STRING_INIT);
}

int cgi_UI_GetMenuInfo(char *menuFile,int *x,int *y,int *w,int *h)
{
	return(int) syscallBack(CG_UI_GETMENUINFO,menuFile,x,y,w,h);
}
...
{
	return(int) syscallBack(CG_UI_GETITEMINFO,menuFile,itemName,x,y,w,h,color,background);
}

int cgi_UI_GetItemText(char *menuFile,char *itemName, char* text)
{
	return(int) syscallBack(CG_UI_GETITEMTEXT,menuFile,itemName,text);
}

int cgi_SP_GetStringTextString(const char *text, char *buffer, int bufferLength)
{
	return syscallBack( CG_SP_GETSTRINGTEXTSTRING, text, buffer, bufferLength );
}

int cgi_ANDROID_SetLookScale(int value)
{
	return syscallBack( CG_ANDROID_SET_LOOK_SCALE, value);
}
int cgi_EndGame(void)
{
	return syscallBack( CG_SENDCONSOLECOMMAND, "cam_disable; disconnect\n" );//; cinematic outcast
}
