...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	SDL_GL_SetAttribute( SDL_GL_BUFFER_SIZE, glConfig.colorBits);
	SDL_GL_SetAttribute( SDL_GL_DEPTH_SIZE,glConfig.depthBits );
	SDL_GL_SetAttribute( SDL_GL_STENCIL_SIZE, 8 );

	if (!(vidscreen = SDL_SetVideoMode(glConfig.vidWidth, glConfig.vidHeight, glConfig.colorBits, SDL_FULLSCREEN | SDL_OPENGL)))
	{
		ri.Printf( PRINT_DEVELOPER, "SDL_SetVideoMode failed: %s\n", SDL_GetError( ) );
	}
	else
	{
		ri.Printf( PRINT_ALL, "SDL_SetVideoMode OK\n" );
	}
	if (!vidscreen)
	{
...
#ifdef __ANDROID__ //Actually my SDL wants Opengl
	flags = SDL_OPENGL;
#endif
