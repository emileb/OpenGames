...
//#ifndef __ANDROID__
	...
	else
	{
		...
		{
			Com_Printf( "Network search = 0x%08x\n",search);
			if (!search->ifa_name)
				continue;

			if (!strlen(search->ifa_name))
				break;
			...
			Com_Printf( "Network IF name %s\n",search->ifa_name);
		}
		Com_Printf( "Freeing ifpa (now NOT freeing)");
		//Caused a crash on someones device, don't bother freeing as pretty small anyway
		//freeifaddrs(ifap);
		Com_Printf( "Calling Sys_ShowIP");
		Sys_ShowIP();
	}
//#endif
}
