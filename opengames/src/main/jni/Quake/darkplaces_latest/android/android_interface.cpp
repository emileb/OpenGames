...
void gameButton(int state,int code)
{
	...
	else if  (code == KEY_PREV_WEAPONS)
	{
		...
		PortableAction(state,PORT_MALICE_RELOAD);
	}
	else if  (code ==KEY_MALICE_USE){
		PortableAction(state,PORT_MALICE_USE);
	}
	else if  (code ==KEY_MALICE_CYCLE){
		PortableAction(state,PORT_MALICE_CYCLE);
	}
...
{
	if (code == K_F9) //bit of a hack
		PortableKeyEvent(state, code);
	else if (code == KEY_SHOW_KBRD && state)
		showKeyboard(1);
	else
...
void blankButton(int state,int code)
{
	...
	PortableKeyEvent(state, K_SPACE);
}
...
void initControls(int width, int height,const char * graphics_path,const char *settings_file,int game)
{
	...
	if (!controlsCreated)
	{
		...
		tcMenuMain->addControl(new touchcontrols::Button("quick_load",touchcontrols::RectF(20,0,22,2),"load",K_F9));
		tcMenuMain->addControl(new touchcontrols::Button("enter",touchcontrols::RectF(0,12,4,16),"enter",PORT_ACT_MENU_SELECT));
		tcMenuMain->addControl(new touchcontrols::Button("keyboard",touchcontrols::RectF(0,0,2,2),"keyboard",KEY_SHOW_KBRD));
		...
		if (game != 1) //Malice does not have prev weapon!
		{
		...
		if (game == 1) //Malice
		{
			tcGameMain->addControl( new touchcontrols::Button("next_weapon",touchcontrols::RectF(0,4,3,6),"next_weap",KEY_NEXT_WEAPONS));
			tcGameMain->addControl(new touchcontrols::Button("malice_reload",touchcontrols::RectF(0,6,3,9),"reload",KEY_MALICE_RELOAD));
		...
		std::string file = (std::string)graphics_path +  "/darkplaces";
		file += game;
		file += ".xml";
		tcGameMain->setXMLFile(file);
