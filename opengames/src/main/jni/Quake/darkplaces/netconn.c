...
#define NETWORK_PREAMBLE "\377\377\377\377"
//#define NETWORK_PREAMBLE "XXXX"
...
//cvar_t cl_netport = {0, "cl_port", "0", "forces client to use chosen port number if not 0"};
//cvar_t sv_netport = {0, "port", "25000", "server port for players to connect to"};
//cvar_t net_address = {0, "net_address", "0.0.0.0", "network address to open ipv4 ports on (if empty, use default interfaces)"};
...
static void _ServerList_Test(void)
{
	...
	for( i = 0 ; i < 1024 ; i++ ) {
		...
		dpsnprintf_NETOWRK( serverlist_cache[serverlist_cachecount].info.name, sizeof(serverlist_cache[serverlist_cachecount].info.name), "Black's ServerList Test %i", i );
		serverlist_cache[serverlist_cachecount].finished = true;
		dpsnprintf_NETOWRK( serverlist_cache[serverlist_cachecount].line1, sizeof(serverlist_cache[serverlist_cachecount].info.line1), "%i %s", serverlist_cache[serverlist_cachecount].info.ping, serverlist_cache[serverlist_cachecount].info.name );
...
static void NetConn_OpenClientPort(const char *addressstring, lhnetaddresstype_t addresstype, int defaultport)
{
	...
	//LOGI("NetConn_OpenClientPort %s",addressstring);
	//addressstring = "127.0.0.1:26000";
...
static qboolean NetConn_OpenServerPort(const char *addressstring, lhnetaddresstype_t addresstype, int defaultport, int range)
{
	...
	{
		LOGI("Addr string1 = %s",addressstring);
		//addressstring = "127.0.0.1";
		LOGI("Addr string2 = %s",addressstring);
		if (addressstring && addressstring[0])
...
static void NetConn_ClientParsePacket_ServerList_UpdateCache(int n)
{
	...
	dpsnprintf_NETOWRK(entry->line1, sizeof(serverlist_cache[n].line1), "^%c%5d^7 ^%c%3u^7/%3u %-65.65s", info->ping >= 300 ? '1' : (info->ping >= 200 ? '3' : '7'), (int)info->ping, ((info->numhumans > 0 && info->numhumans < info->maxplayers) ? (info->numhumans >= 4 ? '7' : '3') : '1'), info->numplayers, info->maxplayers, info->name);
	dpsnprintf_NETOWRK(entry->line2, sizeof(serverlist_cache[n].line2), "^4%-21.21s %-19.19s ^%c%-17.17s^4 %-20.20s", info->cname, info->game,
			(
	...
	while (length >= 7)
	{
		...
		if (data[0] == '\\')
		{
			...
			if (port != 0 && (data[1] != 0xFF || data[2] != 0xFF || data[3] != 0xFF || data[4] != 0xFF))
				dpsnprintf_NETOWRK (ipstring, sizeof (ipstring), "%u.%u.%u.%u:%hu", data[1], data[2], data[3], data[4], port);
		...
		else if (data[0] == '/' && isextended && length >= 19)
		{
			...
			if (port != 0)
			{
				...
				if (ifname != NULL)
				{
					dpsnprintf_NETOWRK (ipstring, sizeof (ipstring), "[%x:%x:%x:%x:%x:%x:%x:%x%%%s]:%hu",
...
#endif
				{
					dpsnprintf_NETOWRK (ipstring, sizeof (ipstring), "[%x:%x:%x:%x:%x:%x:%x:%x]:%hu",
								(data[1] << 8) | data[2], (data[3] << 8) | data[4], (data[5] << 8) | data[6], (data[7] << 8) | data[8],
								(data[9] << 8) | data[10], (data[11] << 8) | data[12], (data[13] << 8) | data[14], (data[15] << 8) | data[16],
								port);
				}
	...
	if (length >= 5 && data[0] == 255 && data[1] == 255 && data[2] == 255 && data[3] == 255)
	{
		...
		switch(Crypto_ClientParsePacket(string, length, senddata+4, &sendlength, peeraddress))
		{
			...
			case CRYPTO_MATCH:
				if(sendlength)
				{
					memcpy(senddata, NETWORK_PREAMBLE, 4);
		...
		if (length >= 10 && !memcmp(string, "challenge ", 10) && cls.rcon_trying)
		{
			...
			if (j < MAX_RCONS)
			{
				...
				const char *e;
				int n;
				dpsnprintf_NETOWRK(argbuf, sizeof(argbuf), "%s %s", string + 10, cls.rcon_commands[i]);
				memcpy(buf, NETWORK_PREAMBLE"srcon HMAC-MD4 CHALLENGE ", 29);
				...
				if(HMAC_MDFOUR_16BYTES((unsigned char *) (buf + 29), (unsigned char *) argbuf, strlen(argbuf), (unsigned char *) rcon_password.string, n))
				{
					...
					{
						int l;
						NetConn_WriteString(mysocket, NETWORK_PREAMBLE"getchallenge", peeraddress);
		...
		if (length >= 10 && !memcmp(string, "challenge ", 10) && cls.connect_trying)
		{
			...
			NetConn_WriteString(mysocket, va_NETWORK(vabuf, sizeof(vabuf), NETWORK_PREAMBLE"connect\\protocol\\darkplaces 3\\protocols\\%s%s\\challenge\\%s", protocolnames, cls.connect_userinfo, string + 10), peeraddress);
			return true;
		}
		...
		if (!memcmp(string, "d\n", 2) && serverlist_cachecount < SERVERLIST_TOTALSIZE)
		{
			...
			{
				dpsnprintf_NETOWRK (ipstring, sizeof (ipstring), "%u.%u.%u.%u:%u", data[0], data[1], data[2], data[3], data[4] * 256 + data[5]);
				if (serverlist_consoleoutput && developer_networking.integer)
		...
		if (!strncmp(string, "extResponse ", 12))
		{
			...
			dpsnprintf_NETOWRK(cl_net_extresponse[cl_net_extresponse_last], sizeof(cl_net_extresponse[cl_net_extresponse_last]), "\"%s\" %s", addressstring2, string + 12);
			return true;
		}
		if (!strncmp(string, "ping", 4))
		{
			...
			NetConn_WriteString(mysocket, NETWORK_PREAMBLE"ack", peeraddress);
			return true;
		}
		...
		if (length > 1 && string[0] == 'c' && (string[1] == '-' || (string[1] >= '0' && string[1] <= '9')) && cls.connect_trying)
		{
			...
			// update the server IP in the userinfo (QW servers expect this, and it is used by the reconnect command)
			InfoString_SetValue(cls.userinfo, sizeof(cls.userinfo), "*ip", addressstring2);
			NetConn_WriteString(mysocket, va_NETWORK(vabuf, sizeof(vabuf), NETWORK_PREAMBLE"connect %i %i %i \"%s%s\"\n", 28, cls.qw_qport, atoi(string + 1), cls.userinfo, cls.connect_userinfo), peeraddress);
			return true;
		}
		...
		if( entry->querycounter	!=	(unsigned) net_slist_maxtries.integer )
		{
			...
			if	(entry->protocol == PROTOCOL_QUAKEWORLD)
			{
				for (socket	= 0; socket	< cl_numsockets ;	socket++)
					NetConn_WriteString(cl_sockets[socket], NETWORK_PREAMBLE"status\n", &address);
			}
			else
			{
				for (socket	= 0; socket	< cl_numsockets ;	socket++)
					NetConn_WriteString(cl_sockets[socket], NETWORK_PREAMBLE"getstatus", &address);
			}
...
void NetConn_ClientFrame(void)
{
	...
	if (cls.connect_trying && cls.connect_nextsendtime < realtime)
	{
		...
		NetConn_WriteString(cls.connect_mysocket, NETWORK_PREAMBLE"getchallenge", &cls.connect_address);
...
static qboolean NetConn_BuildStatusResponse(const char* challenge, char* out_msg, size_t out_size, qboolean fullstatus)
{
	...
	*qcstatus = 0;
	...
	length = dpsnprintf_NETOWRK(out_msg, out_size,
						NETWORK_PREAMBLE"%s\x0A"
		...
		for (i = 0;i < (unsigned int)svs.maxclients;i++)
		{
			...
			if (cl->active)
			{
				...
				*qcstatus = 0;
				...
				if(*qcstatus)
					length = dpsnprintf_NETOWRK(ptr, left, "%s %d%s \"%s\"\n",
										qcstatus,
										pingvalue,
										teambuf,
										cleanname);
				else
					length = dpsnprintf_NETOWRK(ptr, left, "%d %d%s \"%s\"\n",
										cl->frags,
										pingvalue,
										teambuf,
										cleanname);
		...
		if(l)
		{
			...
			while(COM_ParseToken_Console(&text))
			{
				...
				else // single-arg expression? must match the beginning of the command
				{
					if(!strcmp(com_token, s))
						goto match;
					if(!memcmp(va_NETWORK(vabuf, sizeof(vabuf), "%s ", com_token), s, strlen(com_token) + 1))
						goto match;
				}
			}
...
allow:
	...
	if(have_usernames && userpass_startpass && userpass_startpass < userpass_end)
		return va_NETWORK(vabuf, sizeof(vabuf), "%srcon (username %.*s)", restricted ? "restricted " : "", (int)(userpass_startpass-userpass_start), userpass_start);

	return va_NETWORK(vabuf, sizeof(vabuf), "%srcon", restricted ? "restricted " : "");
}
...
static int NetConn_ServerParsePacket(lhnetsocket_t *mysocket, unsigned char *data, int length, lhnetaddress_t *peeraddress)
{
	...
	if (length >= 5 && data[0] == 255 && data[1] == 255 && data[2] == 255 && data[3] == 255)
	{
		...
		if (length >= 12 && !memcmp(string, "getchallenge", 12) && (islocal || sv_public.integer > -3))
		{
			...
			// send the challenge
			dpsnprintf_NETOWRK(response, sizeof(response), NETWORK_PREAMBLE"challenge %s", challenge[i].string);
			LOGI("RESP STR = %s",response);
		...
		if (length > 8 && !memcmp(string, "connect\\", 8))
		{
			...
			if(!(islocal || sv_public.integer > -2))
			{
				...
				NetConn_WriteString(mysocket, va_NETWORK(vabuf, sizeof(vabuf), NETWORK_PREAMBLE"reject %s", sv_public_rejectreason.string), peeraddress);
				return true;
			}
			...
			if(!(s = InfoString_GetValue(string, "protocol", infostringvalue, sizeof(infostringvalue))) || strcmp(s, "darkplaces 3"))
			{
				...
				NetConn_WriteString(mysocket, NETWORK_PREAMBLE"reject Wrong game protocol.", peeraddress);
				return true;
			}
				...
				if (client->netconnection && LHNETADDRESS_Compare(peeraddress, &client->netconnection->peeraddress) == 0)
				{
					...
					if(crypto && crypto->authenticated)
					{
						...
						if(client->netconnection->crypto.authenticated)
						{
							  ...
							  )
							{
								...
								NetConn_WriteString(mysocket, NETWORK_PREAMBLE"reject Attempt to change key of crypto.", peeraddress);
								return true;
							}
						}
					}
					else
					{
						...
						if(client->netconnection->crypto.authenticated)
						{
							...
							NetConn_WriteString(mysocket, NETWORK_PREAMBLE"reject Attempt to downgrade crypto.", peeraddress);
							return true;
						}
					}
					if (client->begun)
					{
						...
						NetConn_WriteString(mysocket, NETWORK_PREAMBLE"accept", peeraddress);
						if(crypto && crypto->authenticated)
			...
			NetConn_WriteString(mysocket, NETWORK_PREAMBLE"reject Server is full.", peeraddress);
		...
		if (!strncmp(string, "extResponse ", 12))
		{
			...
			dpsnprintf_NETOWRK(sv_net_extresponse[sv_net_extresponse_last], sizeof(sv_net_extresponse[sv_net_extresponse_last]), "'%s' %s", addressstring2, string + 12);
			return true;
		}
		if (!strncmp(string, "ping", 4))
		{
	...
	if (length >= 5 && (i = BuffBigLong(data)) && (i & (~NETFLAG_LENGTH_MASK)) == (int)NETFLAG_CTL && (i & NETFLAG_LENGTH_MASK) == length && (sv.protocol == PROTOCOL_QUAKE || sv.protocol == PROTOCOL_QUAKEDP || sv.protocol == PROTOCOL_NEHAHRAMOVIE || sv.protocol == PROTOCOL_NEHAHRABJP || sv.protocol == PROTOCOL_NEHAHRABJP2 || sv.protocol == PROTOCOL_NEHAHRABJP3 || sv.protocol == PROTOCOL_DARKPLACES1 || sv.protocol == PROTOCOL_DARKPLACES2 || sv.protocol == PROTOCOL_DARKPLACES3) && !ENCRYPTION_REQUIRED)
	{
		...
		case CCREQ_CONNECT:
			...
			if(!(islocal || sv_public.integer > -2))
			{
				...
				MSG_WriteString(&sv_message, va_NETWORK(vabuf, sizeof(vabuf), "%s\n", sv_public_rejectreason.string));
		...
		for (i = 0;i < cl_numsockets;i++)
		{
			if (cl_sockets[i])
			{
				...
				if(LHNETADDRESS_GetAddressType(&broadcastaddress) == af)
				{
					...
					NetConn_WriteString(cl_sockets[i], NETWORK_PREAMBLE"getstatus", &broadcastaddress);
				}
				...
				dpsnprintf_NETOWRK(request, sizeof(request), NETWORK_PREAMBLE"%s %s %u empty full%s", cmdname, gamename, NET_PROTOCOL_VERSION, extraoptions);
		...
		for (i = 0;i < cl_numsockets;i++)
		{
			if (cl_sockets[i])
			{
				...
				if(LHNETADDRESS_GetAddressType(&broadcastaddress) == af)
				{
					...
					NetConn_WriteString(cl_sockets[i], NETWORK_PREAMBLE"status\n", &broadcastaddress);
					...
					dpsnprintf_NETOWRK(request, sizeof(request), "c\n");
				}
	...
	if (sv.active && sv_public.integer > 0 && svs.maxclients >= 2 && (priority > 1 || realtime > nextheartbeattime))
	{
		...
		for (masternum = 0;sv_masters[masternum].name;masternum++)
			if (sv_masters[masternum].string && sv_masters[masternum].string[0] && LHNETADDRESS_FromString(&masteraddress, sv_masters[masternum].string, DPMASTER_PORT) && (mysocket = NetConn_ChooseServerSocketForAddress(&masteraddress)))
				NetConn_WriteString(mysocket, NETWORK_PREAMBLE"heartbeat DarkPlaces\x0A", &masteraddress);
	}
}
