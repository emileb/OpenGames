...
static void Host_SaveConfig_to(const char *file)
{
	...
	LOGI("Host_SaveConfig_to %s",file);
...
void Host_Shutdown(void)
{
	...
	Log_Close();
	Crypto_Shutdown();
	LOGI("Host_Shutdown 1");
	Host_UnlockSession();
	LOGI("Host_Shutdown 2");
	S_Shutdown();
	LOGI("Host_Shutdown 3");
	Con_Shutdown();
	LOGI("Host_Shutdown 4");
#ifndef ANDROID
	Memory_Shutdown(); //Causes frame not to return
#endif
	LOGI("Host_Shutdown end");
	usleep(1000 * 500); //Wait half second for files to be written..
}
