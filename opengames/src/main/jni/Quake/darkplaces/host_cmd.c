...
static void Host_Rcon_f (void) // credit: taken from QuakeWorld
{
	...
	if (mysocket && Cmd_Args()[0])
	{
		...
		if(Cmd_Argv(0)[0] == 's' || rcon_secure.integer > 1)
		{
			...
			++cls.rcon_trying;
			if(i >= MAX_RCONS)
				NetConn_WriteString(mysocket, NETWORK_PREAMBLE"getchallenge", &to); // otherwise we'll request the challenge later
		...
		else if(rcon_secure.integer > 0)
		{
			...
			memcpy(buf, NETWORK_PREAMBLE"srcon HMAC-MD4 TIME ", 24);
		...
		{
			NetConn_WriteString(mysocket, va(vabuf, sizeof(vabuf),NETWORK_PREAMBLE"rcon %.*s %s", n, rcon_password.string, Cmd_Args()), &to);
		}
	}
}
