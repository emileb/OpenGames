...
void SV_WipeSavegame (char *savename)
{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/%s/server.ssv", FS_Gamedir (),savegamePath, savename);
	remove (name);
	Com_sprintf (name, sizeof(name), "%s/%s/%s/game.ssv", FS_Gamedir (),savegamePath, savename);
	remove (name);

	Com_sprintf (name, sizeof(name), "%s/%s/%s/*.sav", FS_Gamedir (),savegamePath, savename);
	...
	Sys_FindClose ();
	Com_sprintf (name, sizeof(name), "%s/%s/%s/*.sv2", FS_Gamedir (),savegamePath, savename);
	s = Sys_FindFirst(name, 0, 0 );
	while (s)
	{
		remove (s);
...
void SV_CopySaveGame (char *src, char *dst)
{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/%s/server.ssv", FS_Gamedir(),savegamePath, src);
	Com_sprintf (name2, sizeof(name2), "%s/%s/%s/server.ssv", FS_Gamedir(),savegamePath, dst);
	FS_CreatePath (name2);
	CopyFile (name, name2);

	Com_sprintf (name, sizeof(name), "%s/%s/%s/game.ssv", FS_Gamedir(),savegamePath, src);
	Com_sprintf (name2, sizeof(name2), "%s/%s/%s/game.ssv", FS_Gamedir(),savegamePath, dst);
	CopyFile (name, name2);

	Com_sprintf (name, sizeof(name), "%s/%s/%s/", FS_Gamedir(),savegamePath, src);
	len = strlen(name);
	Com_sprintf (name, sizeof(name), "%s/%s/%s/*.sav", FS_Gamedir(),savegamePath, src);
	found = Sys_FindFirst(name, 0, 0 );
	while (found)
	{
		strcpy (name+len, found+len);

		Com_sprintf (name2, sizeof(name2), "%s/%s/%s/%s", FS_Gamedir(),savegamePath, dst, found+len);
		CopyFile (name, name2);
...
void SV_WriteLevelFile (void)
{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/current/%s.sv2", FS_Gamedir(),savegamePath, sv.name);
	f = fopen(name, "wb");
	if (!f)
	{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/current/%s.sav", FS_Gamedir(),savegamePath, sv.name);
	ge->WriteLevel (name);
}
...
void SV_WriteServerFile (qboolean autosave)
{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/current/server.ssv", FS_Gamedir(),savegamePath);
	f = fopen (name, "wb");
	if (!f)
	{
	...
	// write game state
	Com_sprintf (name, sizeof(name), "%s/%s/current/game.ssv", FS_Gamedir(),savegamePath);
...
void SV_GameMap_f (void)
{
	...
	FS_CreatePath (va("%s/%s/current/", FS_Gamedir(),savegamePath));
...
void SV_Loadgame_f (void)
{
	...
	Com_sprintf (name, sizeof(name), "%s/%s/%s/server.ssv", FS_Gamedir(),savegamePath, Cmd_Argv(1));
	f = fopen (name, "rb");
	if (!f)
	{
